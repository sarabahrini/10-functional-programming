...
(function (module) {
  ...
  function Article(rawDataObj) {
    ...
    //Note for my self the arrow here makes us don't need the this anymore to be added at the end and capitol O for the object makes it constructor function 
  ...
  Article.numWordsAll = () => {
    // Note-SB: Go through each article
    // Turn body of articles into arrays
    //Note-SB: Grab  number of words in each 
    // ANote-SB: dd the word count of each article together

    // const wordCountArray = [];
    const reducer = (accumulator, currentValue) => accumulator + currentValue;

    function wordCount() {
      const count = Article.body.split(' ').length;
    }
    return Article.all.map(wordCount).reduce(reducer);
  };
  ...
  Article.prototype.insertRecord = function (callback) {
    ...
    // Note for my self we can not sue arrows with prototypes because they have lexical scope. meaning the scope in only inside of the function. 
